...
from opendrive2lanelet.opendriveparser.elements.opendrive import OpenDrive, Header
...
from opendrive2lanelet.opendriveparser.elements.roadtype import (
    RoadType,
...
from opendrive2lanelet.opendriveparser.elements.roadLateralProfile import (
...
from opendrive2lanelet.opendriveparser.elements.roadLanes import (
    ...
    RoadMark as RoadLaneSectionRoadMark,
    Speed as RoadLaneSectionSpeed
)
    ...
    for sideTag, newSideLanes in sides.items():
        ...
        for lane in side.findall("lane"):
            ...
            for markIdx, roadMark in enumerate(lane.findall("roadMark")):
                ...
                newMark = RoadLaneSectionRoadMark(
                    color=roadMark.get("color"),
                    laneChange=roadMark.get("laneChange"),
                    material=roadMark.get("material"),
                    mark_type=roadMark.get("type"),
                    weight=roadMark.get("weight"),
                    width=roadMark.get("width"),
                    idx=markIdx,
                    start_offset=float(roadMark.get("sOffset")),
                )
                new_lane.roadMarks.append(newMark)
            ...
            if lane.find("speed") is not None:
                ...
                new_lane.speed = RoadLaneSectionSpeed(
                    max_speed=lane.find("speed").get("max"),
                    unit=lane.find("speed").get("unit"),
                )
            else:
                new_lane.speed = RoadTypeSpeed()
...
def parse_opendrive_header(opendrive, header):
    ...
    if header.findall("userData") is not None:
        for userdata in header.findall("userData"):
            if userdata.get("code") =="mapId":
                parsed_header.mapId = userdata.get("value")
            elif userdata.get("code") =="originLat":
                parsed_header.originLat = userdata.get("value")
            elif userdata.get("code") =="originLong":
                parsed_header.originLong = userdata.get("value")
            elif userdata.get("code") =="originAlt":
                parsed_header.originAlt = userdata.get("value")
            elif userdata.get("code") =="originHdg":
                parsed_header.originHdg = userdata.get("value")
