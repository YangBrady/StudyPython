...
class Lane:
    ...
    def roadMarks(self):
        """ """
        self._roadMarks.sort(key=lambda x: x.start_offset)
    ...
    def getroadMarks(self, markIdx):
        """
        # 将对应的roadMark添加到对应的index位置
        Args:
          widthIdx:
        ...
        """
        for mark in self._roadMarks:
            if mark.idx == markIdx:
                return mark

        return None

    def getLastroadMarkIdx(self):
        """Returns the index of the last width sector of the lane"""

        numRoadMarks = len(self._roadMarks)

        if numRoadMarks > 1:
            return numRoadMarks - 1
...
"""
    ...
    def __init__(
        ...
        self.weight = weight
        self.width = width
        super().__init__(self, start_pos=start_offset)
